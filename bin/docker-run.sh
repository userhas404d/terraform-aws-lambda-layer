...
function run_docker() {
    echo "run_docker: Parsing docker run configuration"
    ...
    echo "run_docker: Running docker"
    echo "run_docker: docker run --rm -i ${DOCKER_BINDMOUNTS_ARRAY[@]} -w \"$BINDMOUNT_ROOT\" --cap-add=SYS_PTRACE \"$IMAGE_NAME\" $LAYER_BUILD_COMMAND"
